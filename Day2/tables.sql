...
Insert into users values("arya@gmail.com","arya123",'student');
Insert into users values("tasalima@gmail.com","tasalima@123",'student');
Insert into users values("shravani@gmail.com","shravani@2075",'student');
...
Insert into students(reg_no,name,email,course_id,mobile_no) values (1,'arya','arya@gmail.com',1,934797394);
Insert into students(reg_no,name,email,course_id,mobile_no) values (2,'shravani','shravani@gmail.com',2,878489294);
Insert into students(reg_no,name,email,course_id,mobile_no) values (3,'tasalima','tasalima@gmail.com',2,973909424);
...
1) Select * from courses where start_date > CURDATE();


2) Select s.reg_no,s.name,s.email,s.mobile_no,c.course_id,c.course_name from students s INNER JOIN courses c ON s.course_id=c.course_id;


3) Select s.reg_no,s.name,s.email,s.mobile_no,c.course_id,c.course_name,c.description,c.fees,c.start_date,c.end_date,c.video_expiry_day
from students s INNER JOIN courses c ON s.course_id=c.course_id WHERE s.email = 'arya@gmail.com';

4) Select c.course_id,c.course_name,c.start_date,c.end_date,c.video_expiry_day,
v.video_id,v.title,v.added_at from students s INNER JOIN courses c on s.course_id = c.course_id 
LEFT JOIN videos v on c.course_id = v.course_id and DATE_ADD(v.added_at, INTERVAL c.video_expiry_day DAY) >= CURDATE()
where s.email = 'arya@gmail.com';